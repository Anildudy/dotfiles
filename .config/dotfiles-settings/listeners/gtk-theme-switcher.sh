...
apply_theme() {
    ...
    if [[ "$THEME_PREF" == "1" || "$THEME_PREF" == "true" ]]; then
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/dotfiles/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        else
            matugen image $(cat ~/.cache/dotfiles/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        fi
        ...
        # Update App Dotfiles Settings theme
        qs -p $HOME/.local/share/dotfiles-settings-app/quickshell ipc call theme-manager reload
        echo "App Dotfiles Settings Theme updated"
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/dotfiles/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        else
            matugen image $(cat ~/.cache/dotfiles/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        fi
